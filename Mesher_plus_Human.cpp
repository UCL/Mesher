...
int main()
{
  ...
  sizing_field_elliptic size_e_coarse (or);
  ...
  size_e_coarse.fine_size=5*p.cs;
  size_e_coarse.coarse_size=size_e_coarse.fine_size;
  size_e_coarse.preserve=0;
  ...
  Mesh_criteria criteria_coarse(facet_angle=p.fa, facet_size=size_e_coarse, facet_distance=p.fd,
                         cell_radius_edge_ratio=p.cre, cell_size=size_e_coarse);
  // Meshing
  std::cout<<"\n Meshing with initial mesh...";

  C3t3 c3t3= CGAL::make_mesh_3<C3t3>(domain, criteria_coarse, CGAL::parameters::features(domain), 
